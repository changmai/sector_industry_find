...
const TickList: React.FC<TickListProps> = ({ ticks }) => {
  return (
    <div className="flex flex-col h-full bg-panel-bg rounded-lg border border-border-color overflow-hidden">
        ...
        {/* Grid: Time(50px) | Price(60px) | Side(26px) | Vol(48px) */}
        <div className="grid grid-cols-[50px_60px_26px_48px] gap-1 text-[9px] text-gray-500 px-1 py-0.5 border-b border-border-color items-center">
            <div className="text-center">Time</div>
            <div className="text-right">Price</div>
            <div className="text-center">Sd</div>
            <div className="text-right pr-1">Vol</div>
        </div>

        {/* List */}
        <div className="flex-1 overflow-y-auto custom-scrollbar">
            {ticks.map((tick) => (
                <div key={tick.id} className="grid grid-cols-[50px_60px_26px_48px] gap-1 text-[10px] px-1 py-0.5 border-b border-gray-800/30 hover:bg-gray-800/50 font-mono animate-fade-in leading-none items-center">
                    ...
                    <div className={`text-right pr-1 ${tick.volume >= 100 ? 'text-yellow-400 font-bold' : 'text-gray-300'}`}>
