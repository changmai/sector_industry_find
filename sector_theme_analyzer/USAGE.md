...
| **B** | N일 신고거래대금 | `days` | 60일 |
| **C** | 거래량 회전율 상위 N종목 | `min_rate`, `top_n` | 0%, 100개 |
| **D** | 이평 정배열 (단기>중기>장기) | `short`, `mid`, `long` | 20, 60, 120 |
| **E** | N일 신고가 대비 등락률 범위 | `days`, `min_pct`, `max_pct` | 60일, -5%, 0% |
| **F** | N일전 대비 거래량 비율 | `compare_days`, `min_ratio`, `max_ratio` | 5일, 150%, 9000% |
| **G** | 회전율 X%~Y% 범위 | `min_rate`, `max_rate` | 1%, 100% |
...
#### 조건 B: N일 신고거래대금
- **로직**: 해당일 거래대금이 최근 N일 중 최고인 종목
- **파라미터**:
  - `days`: 비교할 일수 (기본 60일)
- **위치**: `condition_engine.py:298-340`
...
- **위치**: `condition_engine.py:345-380`
...
- **위치**: `condition_engine.py:385-440`

#### 조건 E: N일 신고가 대비 등락률
- **로직**: 현재가가 최근 N일 신고가 대비 특정 범위 내인 종목
- **파라미터**:
  - `days`: 비교할 일수 (기본 60일)
  - `min_pct`: 최소 등락률 (기본 -5%)
  - `max_pct`: 최대 등락률 (기본 0%)
- **예시**: 신고가 대비 -5% ~ 0% 범위 = 신고가 근처 종목
- **위치**: `condition_engine.py:458-505`

#### 조건 F: 거래량 비교
- **로직**: (현재 5일 평균) / (N일전 5일 평균) × 100
- **파라미터**:
  - `compare_days`: 비교 기준 일수 간격 (기본 5일)
  - `min_ratio`: 최소 비율 (기본 150%)
  - `max_ratio`: 최대 비율 (기본 9000%)
- **예시**: 150% = 거래량이 1.5배 이상 증가
- **위치**: `condition_engine.py:507-558`

#### 조건 G: 회전율 범위
- **로직**: 당일 회전율이 X% ~ Y% 범위 내인 종목
- **파라미터**:
  - `min_rate`: 최소 회전율 % (기본 1%)
  - `max_rate`: 최대 회전율 % (기본 100%)
- **예시**: 47% ~ 100% 범위 = 고회전율 종목
- **위치**: `condition_engine.py:560-602`
